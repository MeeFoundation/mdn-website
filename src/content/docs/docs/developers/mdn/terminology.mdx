...
An application that allows the user to control their [Personal Data](#personal-data) and [Identity Agent's](#identity-agent) behavior.
...
A [service provider application](#service-provider-application) or a .
...
An application that provides [Agent App](#agent-app) functionality and [MDN Node](#mdn-node) functionality in a form of [Local App](#local-app).
...
A website, web service, mobile, tablet, laptop, or desktop application provided by a [Service Provider](#service-provider). A Service Provider Application must provide an [MDN Node](#mdn-node) functionality.
...
A [corporation](#corporation) that provides a [Service Provider Application](#service-provider-application) for its users.
